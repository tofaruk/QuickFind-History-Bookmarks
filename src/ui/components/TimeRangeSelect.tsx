...
export function TimeRangeSelect({ value, onChange }: TimeRangeSelectProps) {
  ...
  const options = useMemo<TimeRange[]>(() => {
    const past = Array.from({ length: 8 }, (_, i) => ({ kind: "pastWeeks", weeks: i + 1 } as const))
    ...
    <div className="relative">
      ...
      >
        <span className="min-w-0 truncate">
          ...
          <span className="font-medium text-gray-900">{labelFor(value)}</span>
        </span>
            ...
            {options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "flex w-full items-center justify-between rounded-xl px-2 py-2 text-left text-sm hover:bg-gray-50",
                    active ? "bg-gray-50" : "",
                  ].join(" ")}
                >
                  <span className="font-medium text-gray-900">{labelFor(opt)}</span>
